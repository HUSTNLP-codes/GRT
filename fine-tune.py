...
from modeling.modeling_grt import *
...
def evaluate_accuracy(eval_set, model):
    ...
    with torch.no_grad():
        for qids, labels, *input_data in tqdm(eval_set):
            logits, _, __ = model(*input_data, qids=qids)
...
def main():
    ...
    parser.add_argument('--save_dir', default=f'./saved_models/finetune', help='model output directory')
    ...
    parser.add_argument('--checkpoint_dir', default=f'./saved_models/pretrain/model.pt.4')
    ...
    parser.add_argument('--use_cache', default=True, type=bool_flag, nargs='?', const=False, help='use cached data to accelerate data loading')
    ...
    # optimization
    parser.add_argument('-dlr', '--decoder_lr', default=1e-2, type=float, help='d_learning rate')
    ...
    parser.add_argument('--refreeze_epoch', default=60, type=int)
    ...
    parser.add_argument('--add_nodefeatsim', default=False, type=bool_flag, help='')
...
def train(args):
    ...
    if args.dataset == 'medqa_usmle':
        ...
        dataset = LM_GRT_DataLoader(args, args.train_statements, args.train_adj,
                                                args.dev_statements, args.dev_adj,
                                                args.test_statements, args.test_adj,
                                                batch_size=args.batch_size, eval_batch_size=args.eval_batch_size,
                                                device=(device0, device1),
    ...
    #   Build model                                                                                   #
    ###################################################################################################
    model = LM_GRT(args, args.encoder, k=args.k, n_ntype=4, n_etype=args.num_relation,
    ...
    for k in ckpt_dict.keys():
        if k not in model_dict.keys() or k.startswith('encoder'):
            temp.append(k)
    for elem in temp:
        del ckpt_dict[elem]
            ...
            if not args.without_amp:
                for a in range(0, bs, args.mini_batch_size):
                    ...
                    with torch.cuda.amp.autocast():
                        logits, rpe, extra_loss = model(*[x[a:b] for x in input_data], qids=qids[a:b])
...
def eval_detail(args):
    ...
    model = LM_GRT(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation,
