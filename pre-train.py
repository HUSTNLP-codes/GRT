...
from modeling.modeling_grt_pretrain import *
...
def main():
    ...
    parser.add_argument('--save_dir', default=f'./saved_models/pretrain', help='model output directory')
    ...
    # optimization
    parser.add_argument('-dlr', '--decoder_lr', default=1e-5, type=float, help='d_learning rate')
    ...
    for epoch_id in range(args.n_epochs):
        ...
        for qids, labels, *input_data in dataset.train():
            ...
            if not args.without_amp:
                ...
                for a in range(0, bs, args.mini_batch_size):
                    b = min(a + args.mini_batch_size, bs)
                    loss = model(*[x[a:b] for x in input_data], qids=qids[a:b])
                    loss = loss * (b - a) / bs
                    loss.backward()
                    total_loss += loss.item()
                if args.max_grad_norm > 0:
