...
def multi_head_attention_forward(
    ...
    if matched_index is not None and matched_index.size(-1) == 2:
        ...
        if rpe_2:
            ...
            # cls doesn't match any entity, and vice versa
            rpe = rpe.view(-1,rpe.shape[2])  # (20*147) *147
            rpe = rpe.transpose(0,1)  #147*(20*147)
            rpe[0] = 0 #(20*147)
            rpe = rpe.transpose(0,1)  # (20*147)*147
            rpe = rpe.view(bsz,-1,rpe.shape[1]) #20*147*147
            rpe = rpe.transpose(1,2) #20*147*147
            rpe = rpe.contiguous().view(-1,rpe.shape[2]) #(20*147)*147
            rpe = rpe.transpose(0, 1)
            rpe[0] = 0  # (20*147)
            rpe = rpe.transpose(0, 1)
            rpe = rpe.view(bsz, -1, rpe.shape[1])
